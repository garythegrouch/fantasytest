...
def season_scraper(season):
    ...
    #exporting individual team data for each season into different folders and doing some formatting
    ...
    for team, df in all_team_dicts.items():
        ...
        df = df[['GW','h_a','xG','scored', 'xGA', 'missed', 'xpts', 'pts', 'npxG', 'npxGA','ppda', 'ppda_allowed', 'ppda_coef', 'oppda_coef', 'deep',
           'deep_allowed', 'npxGD']]
        df.rename(columns = {'missed': 'conceded',
                            'h_a': 'Home/Away'}, inplace = True)
        outname_team = '{}_{}_season data.csv'.format(team, season)
        outdir_team = r'C:\Users\grema\testfolder'.format(season)
